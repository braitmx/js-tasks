...
function encodeToRot13(str) {
    ...
    var newStr = '', example = 'ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz';

    var foo = function(str) {
        var firstChar = str.charAt(0), index = example.indexOf(firstChar);
        ...
        if (str.length === 0) return newStr; 
            ...
            else if (index <13 || index >= 26 && index < 39){
                index += 13; 
                newStr += example.charAt(index);
                foo(str.slice(1));
            } else {
                index -= 13; 
                newStr += example.charAt(index);
...
function getCardId(value) {
  var result = 'A234567891JQK'.indexOf(value.charAt(0));  
